...
_analyzer.py
------------
Core neurite morphology analysis pipeline.

Can be used as a library (accepting numpy arrays) or via the CLI wrapper
(analyze_neurites.py) that handles file I/O.
"""
...
class NeuriteAnalyzer:
    ...
    def _create_soma_labels(self, soma_list: list[dict], shape: tuple) -> np.ndarray:
        """Return int32 label array: 0=background, 1..N = soma index."""
        labels = np.zeros(shape, dtype=np.int32)
        for i, s in enumerate(soma_list):
            rr, cc = disk((s["y"], s["x"]), s["radius"], shape=shape)
            labels[rr, cc] = i + 1
        return labels
    ...
    # ------------------------------------------------------------------
    # Array-based API (for napari integration)
    # ------------------------------------------------------------------

    def analyze_array(
        self,
        soma_array: np.ndarray,
        neurite_array: Optional[np.ndarray] = None,
    ) -> tuple[np.ndarray, np.ndarray, pd.DataFrame]:
        """
        Run the full analysis pipeline on pre-loaded numpy arrays.

        This is the napari-friendly entry point: it accepts image data already
        loaded into the viewer and returns numpy arrays suitable for adding as
        napari layers, without any file I/O.

        Parameters
        ----------
        soma_array:
            Image array used for soma detection. Any dimensionality supported
            by _extract_2d (2-D, 3-D channel-first/last, 4-D Z-stack).
        neurite_array:
            Optional separate image for neurite detection. When ``None`` the
            soma array is reused for both channels.

        Returns
        -------
        soma_labels : np.ndarray (int32, H×W)
            Label image: 0 = background, 1…N = individual soma.
        skeleton : np.ndarray (bool, H×W)
            One-pixel-wide neurite skeleton.
        df : pd.DataFrame
            Per-soma measurements table (empty if no soma were detected).
        """
        soma_plane = _extract_2d(soma_array, self.soma_channel)
        soma_plane = _normalize(soma_plane)

        if neurite_array is not None:
            neurite_plane = _extract_2d(neurite_array, self.neurite_channel)
            neurite_plane = _normalize(neurite_plane)
        else:
            neurite_plane = soma_plane.copy()
        ...
        if soma_plane.size == 0 or min(soma_plane.shape) < 50:
            raise ValueError(
                "Image is too small for analysis (minimum dimension: 50 px)."
            )
        ...
        soma_labels = self._create_soma_labels(soma_list, soma_plane.shape)

        if not soma_list:
            skeleton = np.zeros(soma_plane.shape, dtype=bool)
            return soma_labels, skeleton, pd.DataFrame()
        ...
        # Partition skeleton pixels to nearest soma (pixel-wise Voronoi)
        ...
        df = pd.DataFrame(measurements) if measurements else pd.DataFrame()
        if not df.empty:
            df["pixel_size_um"] = self.pixel_size_um

        return soma_labels, skeleton, df

    # ------------------------------------------------------------------
    # Per-image orchestration (file-based)
    # ------------------------------------------------------------------
    ...
    def analyze_image(self, path: Path) -> pd.DataFrame:
        ...
        if self.save_viz:
            if self.output_dir is None:
                log.warning("  output_dir not set — skipping visualization")
            else:
                viz_path = self.output_dir / (path.stem + "_analyzed.png")
                self.save_visualization(
                    soma_prep, soma_list, skeleton, viz_branch_df,
                    measurements, all_graphs, viz_path,
                )
    ...
    def run(self) -> pd.DataFrame:
        ...
        if self.image_dir is None:
            raise ValueError("image_dir must be set to use the batch runner.")
